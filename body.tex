...
This results in upper limitation of 196,627 [req/s] when the date size of HTTP response body is 13 byte that was typical data size of our experiment. 
...
since the flanneld is the user space single thread program(NEEDS VERIFICATION!). 
...
Figure~\ref{fig:latency_cdf_rps_40pods} compares Cumulative Distribution Function(CDF) of the load balancer latency at constant load.
...
Future work includes, 1) discussing the load balancer redundancy, 
2) feasibility study in the faster network environment such as 10Giga bit etherenet 
and 3) direct comparison with cloud provider's load balancers.
