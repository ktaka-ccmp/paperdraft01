...
Recently, the Linux container clusters have drawn significant amount of attention because they are lightweight, portable, and repeatable.
Container clusters are generally more lightweight than virtual machine (VM) clusters, 
because the containers share the kernel with the host operationg system (OS), even though they maintain separate execution environments. 
Container clusters are generally portable because the process execution environments are archived into tar files, 
...
This means that container clusters can provide repeatable and portable execution environments.
...
capable of being migrated easily for disaster recovery purposes, or for other business requirements.
...
By using Kubernetes as a middle-ware, users can easily deploy a web service on different 
cloud providers or on on-premise data centers, without adjusting the cluster to the new environment. 
...
This is a typical web service migration senario.
...
Since the Kubernetes fails to provide an uniform environment from a container cluster view point, among the different environments,
...
To accomplish this, we will implement our load balancers by creating containers for Linux kernel's Internet Protocol Virtual Server (IPVS)\cite{Zhang2000} 
Layer 4 load balancer using an existing Kubernetes ingress\cite{K8sIngress2017} framework. 
We will also investigate the feasibility of out proposed load balancer by comparing the performance 
with iptables DNAT load balancer and the Nginx Layer 7 load balancing. 
...
software load balancer containerization, or load balancer tools within the context of the container technology.
...
load balancer capabilities by containerizing the keepalived\cite{ACassen2016} and IPVS. 
The keepalived container use the daemonsets\cite{K8sDaemonsets2017} framework of Kubernetes. 
This work is very similar to our proposal in the sense that it containerizes keepalived and IPVS.
However it differs from our proposal, in the sense that the load balancers are run on on every node 
as a part of Kubernetes infrastructure.
Our proposal aims at providing a portable load balancer that is configurable and deployable at user's will, 
by making it run as a part of the web service itself rather than as part of the Kubernetes cluster infrastructure.  
The swarm mode of the Docker\cite{DockerCoreEngineering2016,DockerInc2017} also uses IPVS for internal load balancing,
but it is also considered as part of Docker's built-in load balancer functionality, 
and thus different from our proposal.
...
2) Distributing the traffic twice, first on the externalload balancers and second on each node, 
...
The proposed load balancer can resolve the common conventional architecture problems, as follows:
...
{\em real servers}\footnote{This term refers to worker servers that will respond to incoming traffic, 
...
to multiple HTTP servers(e.g. Apache HTTP or nginx) running on multiple servers in order to improve the performance of web service.
...
Although keepalived also supports Virtual Router Redundancy Protocol(VRRP)\cite{hinden2004virtual}, that topic is beyond the scope of this paper.
...
and adding CAP\_NET\_ADMIN capability to the container to allow keepalived to manipulate the kerne's IPVS rules. 
...
It is well known that distributing interrupt handling from the network interface card(NIC) 
...
load balancerperformance, in terms of Request/sec can be obtained. 
...
The output example shows information including per thread statistics, error counts, Request/sec and Transfer/sec[MByte/sec].
...
Performance of the load balancers might be influenced by the overhead of encapsulation. 
...
On the other hand, {\tt RSS = off} means that am interrupt from any rx-queue is routed to CPU0. 
It is configured as the following setting:
...
It is not clear what causes the performance limit for the case when \enquote{RPS = on}, 
...
poorest performance level, as the case for the IPVS.
...
Our experimental results indicate that the proposed IPVS based load balancer improves the portability of 
...
We also learned that only the vxlan and udp backend operation modes could be used 
