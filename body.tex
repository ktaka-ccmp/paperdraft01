...
We will also investigate appropriate network settings which enable such loadbalancers to work properly.
...
The upper bound of the performance as a load balancer will not be far better than that of single web server. 
Therefore it is meaningless to use the nginx as a load balancer when one is trying to serve lightweight web pages using
the nginx as a web server.
 
The kube-keepalived-vip\cite{Prashanth2016} project is trying to use the linux kernel's loadbalancer capability, 
called ipvs\cite{Zhang2000} by containerizing the keepalived\cite{ACassen2016}. 
...
This work is very similar in the sense that it uses keepalived and ipvs in container. However it does not provide 
...
\cite{Sibiryov2015}
\cite{Aaltodoc:http://urn.fi/URN:NBN:fi:aalto-201611025433}
\cite{DockerCoreEngineering2016}
\cite{DockerInc2017}
\cite{DockerInc2017}
...
\section{Loadbalancers in Kubernetes}
